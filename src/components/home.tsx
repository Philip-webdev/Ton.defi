...
function DonutChart({ data }: { data: ChartDatum[] }) {
  ...
  const totalLabel = (data.reduce((s, d) => s + d.value, 0) / 1000).toFixed(0);
...
export default function CampusPlanner() {
  ...
  const addToCart = (pkg: FoodPackage): void => {
    setCart((prev) => {
...
7. talk about different dietary constraint on the generated plan.
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (prefers-color-scheme: dark) {
          .app { background: rgb(15,15,15); color: white; }
        }
        ...
        .crypto-pill { display: inline-flex; align-items: center; gap: 6px; background: rgba(51,232,191,0.2); border: 1px solid rgba(51,232,191,0.4); border-radius: 20px; padding: 5px 12px; font-size: 11px; color: rgb(51,232,191); font-weight: 600; margin-top: 4px; }
        ...
        @media (prefers-color-scheme: dark) { .quick-btn { background: rgb(1,1,1); } .quick-btn:hover { box-shadow: 0 6px 20px rgba(51,232,191,0.15); } }
        ...
        @media (prefers-color-scheme: dark) { .pkg-card { background: rgb(1,1,1); } .pkg-card:hover { box-shadow: 0 6px 20px rgba(51,232,191,0.15); } }
        ...
        .vault-icon { font-size: 48px; margin-bottom: 16px; }
        .vault-bal-label { font-size: 11px; color: rgba(255,255,255,0.8); text-transform: uppercase; letter-spacing: 1.5px; margin-bottom: 8px; }
        .vault-bal { font-family: 'Orbitron', sans-serif; font-size: 36px; font-weight: 700; color: white; margin-bottom: 6px; }
        ...
        @media (prefers-color-scheme: dark) { .amt-input { background: rgb(1,1,1); border-color: rgba(51,232,191,0.2); color: white; } .amt-input:focus { border-color: rgb(51,232,191); box-shadow: 0 0 0 3px rgba(51,232,191,0.1); } }
        ...
        .stablecoin-card { background: rgba(51,232,191,0.08); border: 1px solid rgba(51,232,191,0.25); border-radius: 20px; padding: 20px; display: flex; align-items: center; gap: 14px; }
        .sc-icon { font-size: 32px; }
        .sc-info { flex: 1; }
        .sc-label { font-size: 10px; color: rgb(51,232,191); font-weight: 700; text-transform: uppercase; letter-spacing: 1px; }
        .sc-amt { font-family: 'Orbitron', sans-serif; font-size: 22px; font-weight: 700; color: rgb(51,232,191); }
        .sc-sub { font-size: 10px; color: grey; margin-top: 2px; }
        ...
        .plan-cat-item:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(0,131,208,0.1); }
        @media (prefers-color-scheme: dark) { .plan-cat-item { background: rgb(1,1,1); } .plan-cat-item:hover { box-shadow: 0 6px 20px rgba(51,232,191,0.15); } }
        ...
        .plan-input { background: #f5f5f5; border: 1px solid rgba(0,131,208,0.2); border-radius: 10px; padding: 8px 12px; color: rgb(34,34,34); font-size: 12px; font-family: 'Orbitron', sans-serif; font-weight: 600; outline: none; width: 90px; text-align: right; transition: border-color 0.2s; }
        .plan-input:focus { border-color: RGB(0,131,208); }
        @media (prefers-color-scheme: dark) { .plan-input { background: rgb(20,20,20); border-color: rgba(51,232,191,0.2); color: white; } .plan-input:focus { border-color: rgb(51,232,191); } }
        ...
        @media (prefers-color-scheme: dark) { .btn-secondary { border-color: rgba(51,232,191,0.3); color: rgb(51,232,191); } .btn-secondary:hover { background: rgba(51,232,191,0.08); } }
        ...
        .market-card:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(0,131,208,0.15); }
        @media (prefers-color-scheme: dark) { .market-card { background: rgb(1,1,1); } .market-card:hover { box-shadow: 0 6px 20px rgba(51,232,191,0.15); } }
        .market-card .pkg-icon { font-size: 28px; }
        ...
        .add-btn:hover { background: rgba(0,131,208,0.15); }
        @media (prefers-color-scheme: dark) { .add-btn { background: rgba(51,232,191,0.08); border-color: rgba(51,232,191,0.3); color: rgb(51,232,191); } .add-btn:hover { background: rgba(51,232,191,0.15); } }
        ...
        @media (prefers-color-scheme: dark) { .nav-item .nav-icon { color: #555; } .nav-item.active .nav-icon { color: rgb(36,172,242); } .nav-item.active .nav-label { color: rgb(36,172,242); } .nav-item.active { background: rgba(51,232,191,0.08); } }
        ...
        .chip { display: inline-block; background: white; border: 1px solid rgba(0,0,0,0.1); border-radius: 20px; padding: 6px 14px; font-size: 10px; color: grey; cursor: pointer; margin-right: 8px; transition: all 0.2s; font-family: 'Orbitron', sans-serif; }
        .chip.active { background: rgba(0,131,208,0.1); border-color: rgba(0,131,208,0.3); color: RGB(0,131,208); }
        @media (prefers-color-scheme: dark) { .chip { background: rgb(1,1,1); border-color: rgba(255,255,255,0.1); } .chip.active { background: rgba(51,232,191,0.1); border-color: rgba(51,232,191,0.3); color: rgb(51,232,191); } }
          ...
          <div className="screen">
            ...
            <div className="balance-card">
              ...
              {/* <div className="crypto-pill">
                🟢 USDT {hidden ? "••••" : `$${(cryptoBalance / 1650).toFixed(2)}`} · Stablecoin Wallet
              </div> */}
            </div>
            ...
            <div className="quick-grid">
              ...
              <button className="quick-btn" onClick={() => setScreen("Budget")}>
                <span className="qb-icon"><PenSquareIcon/></span><span className="qb-label">Budget</span>
              </button>
              <button className="quick-btn" onClick={() => setScreen("market")}>
                <span className="qb-icon"><ShoppingCart/></span><span className="qb-label">Shop</span>
              </button>
              <button className="quick-btn" onClick={() => setScreen("stats")}>
                <span className="qb-icon"><ChartCandlestickIcon/></span><span className="qb-label">Analytics</span>
              </button>
            </div>
          ...
          <div className="screen">
            ...
            <div className="plan-header-card">
              <div>
                ...
                <div style={{ fontFamily: "Syne", fontSize: "28px", fontWeight: 800, color: "#F0EDE8" }}>₦{totalBudgeted.toLocaleString()}</div>
              </div>
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: "11px", color: "#555", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "4px" }}>Remaining</div>
                <div style={{ fontFamily: "Syne", fontSize: "28px", fontWeight: 800, color: "#3AE87F" }}>₦{(vaultBalance - totalSpent).toLocaleString()}</div>
              </div>
            </div>
        ...
        {screen === "market" && (
          <div className="screen">
            <div style={{display:'flex',justifySelf:'left'}}>
              <button style={{ width:'max-content'}} className="btn-secondary" onClick={() => setScreen("home")}>Back</button>
...
{/* 
            ...
            <div className="market-grid">
              {FOOD_PACKAGES.map((pkg) => (
                <div className="market-card" key={pkg.id}>
                  ...
                  <button className="add-btn" onClick={() => addToCart(pkg)}>
                    + Add to Plan
                  </button>
                </div>
              ))}
            ...
            {cart.length > 0 && (
              <div className="cart-float">
                ...
                <span style={{ color: "white", fontFamily: "Syne", fontWeight: 700 }} onClick={() => navigate('/checkout', {state: {price: {cartTotal}}})}>Pay →</span>
              </div>
            )}
          </div>
        )}
          ...
          <div className="screen">
            ...
            <div className="stats-grid">
              <div className="stat-card">
                ...
                <div className="stat-value dark">₦{(totalBudgeted / 1000).toFixed(0)}k</div>
              </div>
              <div className="stat-card">
                <div className="stat-label">Spent</div>
                <div className="stat-value blue">₦{(totalSpent / 1000).toFixed(0)}k</div>
              </div>
              <div className="stat-card">
                <div className="stat-label">Saved</div>
                <div className="stat-value green">₦{((totalBudgeted - totalSpent) / 1000).toFixed(0)}k</div>
              </div>
              <div className="stat-card">
                <div className="stat-label">Save Rate</div>
                <div className="stat-value green">{Math.round(((totalBudgeted - totalSpent) / totalBudgeted) * 100)}%</div>
              </div>
            </div>
              ...
              <div style={{ display: "flex", gap: "24px", justifyContent: "space-around" }}>
                {[
                  ...
                  <div key={item.label} style={{ textAlign: "center" }}>
                    ...
                    <div style={{ fontFamily: "Syne", fontSize: "22px", fontWeight: 800, color: "#3AE87F" }}>{item.value}</div>
